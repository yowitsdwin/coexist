...
const DailyPhotos = ({ darkMode = false }) => {
  ...
  // This transform function is for photos, not messages
  ...
  return (
    <div className={`rounded-2xl shadow-xl p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className="flex items-center justify-between mb-6">
        <h2 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>📸 Today's Moments</h2>
        <label className="cursor-pointer">
          <input
            ...
            disabled={uploading}
          />
          <div className={`px-4 py-2 rounded-lg flex items-center gap-2 ${uploading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600'} text-white transition-all`}>
      ...
      {recentPhotos.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className={`text-lg ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>No photos today</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {recentPhotos.map((photo) => {
            ...
            return (
              <div key={photo.id} className={`rounded-xl overflow-hidden shadow-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} hover:shadow-2xl transition-shadow`}>
                <div className="relative aspect-square">
                  <img src={photo.imageData} alt="Daily moment" className="w-full h-full object-cover" loading="lazy" />
                  <div className="absolute top-2 right-2 flex gap-2">
                    <button onClick={() => handleDownload(photo.imageData, photo.id)} className="p-2 bg-white/90 backdrop-blur-sm rounded-full hover:bg-white transition-colors shadow-lg" aria-label="Download photo">
                      <Download className="w-4 h-4 text-gray-700" />
                    </button>
                    {isOwnPhoto && (
                      <button onClick={() => handleDelete(photo.id, photo.userId)} className="p-2 bg-red-500/90 backdrop-blur-sm rounded-full hover:bg-red-500 transition-colors shadow-lg" aria-label="Delete photo">
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>{photo.userName || 'Unknown'}</p>
                      <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{formatRelativeTime(photo.timestamp)}</p>
                    </div>
                    <button onClick={() => handleReaction(photo.id, photo.reactions)} className={`flex items-center gap-1 px-3 py-1 rounded-full transition-all ${hasReacted ? 'bg-pink-500 text-white' : darkMode ? 'bg-gray-600 text-gray-300 hover:bg-gray-500' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
      ...
      <p className={`text-xs mt-4 text-center ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Photos automatically delete after 24 hours</p>
    </div>
  );
};
